...
const AddForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const result = await addDeliveryAddress(currentUser?.uid, formValues);
      const updateAddress = await getAddress(currentUser.uid);
      ...
      if (result.exists) {
        ...
        dispatch(setSnackBarMessage("Address added successfully!"));
      }
    } catch (error) {
  ...
  const handleCountryChange = (event, newValue) => {
    setFormValues({ ...formValues, country: newValue ? newValue.label : "" });
  };

  const handleStateChange = (event, newValue) => {
    setFormValues({ ...formValues, state: newValue ? newValue.label : "" });
  };
  // ---

  return (
    <React.Fragment>
      <button className=" add-address-btn " onClick={handleClickOpen}>
        <RiAddLine fontSize="large" /> Add New Address
      </button>
      <Dialog
        open={open}
        onClose={handleClose}
